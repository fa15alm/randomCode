            ...
            for x in range(0, 4):
                for y in range(1, 4):
                    if board[y-1][x] == 0:
                        ...
                        board[y][x] = 0
                    if board[y-1][x] == board[y][x] and not(board[y-1][x] in added or board[y][x] in added):
                        board[y-1][x] = board[y][x]*2
                        score+=board[y][x]*2
                        board[y][x] = 0
                        added.append(board[y-1][x])
        elif direction == 'd':
            for x in range(0, 4):
            ...
            for x in range(1, 4):
                for y in range(0, 4):
                    if board[y][x-1] == 0:
                        ...
                        board[y][x] = 0
                    if board[y][x-1] == board[y][x] and not(board[y][x-1] in added or board[y][x] in added):
                        board[y][x-1] = board[y][x]*2
                        score+=board[y][x]*2
                        board[y][x] = 0
                        added.append(board[y][x-1])
        elif direction == 'r':
    ...
    for x in range(4):
        for y in range(4):
            if board[y][x] == 0:
                return False
            if 0<y<3 and 0<x<3:
                if board[y][x] == board[y-1][x] or board[y][x] == board[y+1][x] or board[y][x] == board[y][x-1] or board[y][x] == board[y][x+1]:
                    return False
            if x == 0:
                if y == 0:
                    if board[y][x] == board[y+1][x] or board[y][x] == board[y][x+1]:
                        return False
                if y == 3:
                    if board[y][x] == board[y-1][x] or board[y][x] == board[y][x+1]:
                        return False
